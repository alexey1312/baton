...
A periodic, opt-in reflection pass that reads usefulness signal from merged pull requests
(👍/👎 reactions plus thread resolution on Baton-authored threads), attributes it per scope,
and proposes edits to the review setup — never source code — as a single rolling draft pull
request, gated by `min_signal` over a `lookback_days` window and constrained to an edit
allowlist. Safe-by-default (preview) and runnable unattended on CI.

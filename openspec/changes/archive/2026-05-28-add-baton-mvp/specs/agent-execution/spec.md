...
Builds and runs external coding-CLI agent invocations uniformly through a single process executor, honoring `binary`/`args` overrides for every agent and sandboxing each run.

...
Tag-triggered GitHub Releases that build multi-platform `baton` binaries, generate a `git-cliff` changelog, attach artifacts, and feed the `mise` (github backend) and Homebrew (`alexey1312/homebrew-tap`) install channels — ported from the ExFig release pipeline.

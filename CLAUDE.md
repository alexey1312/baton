...
- Findings are deduped at **two** levels: intra-task by exact `Finding.DedupeKey` (`ReviewOrchestrator.dedupe`, across diff chunks), then **cross-task** by `CrossTaskDedup.merge` (always-on, in `ReviewCommand` before persistence, so DB/render/GitHub all see the merged set). Cross-task buckets by exact `(file, line)` then fuzzy-matches titles (token-set Jaccard ≥ `FindingMatch.titleThreshold`), folds matches from **different** tasks into one finding tagged `Finding.confirmedBy` (rendered "Confirmed by N reviews" only when ≥2). It never softens the exit code: the survivor takes the group's max severity, and a review that loses a `failOn`-crossing copy sets `ReviewTaskResult.removedCrossingFindings` so its `failed` stays true. The merge is a no-op when no two tasks overlap.
...
- Git hooks live in `.githooks/` (wired via `core.hooksPath`, set by `mise run setup` and the `[hooks] enter` block). `pre-commit`/`commit-msg` route to `hk`; **`pre-push` dogfoods `baton review` itself** via the `prepush-review` mise task — it reviews the commits being pushed (diff vs `@{upstream}`, not the working tree) and **blocks** the push on findings ≥ `fail_on`. Graceful-skips when there's no upstream / nothing to push / `BATON_PREPUSH=0`; bypass with `git push --no-verify`. Binary resolution does **no** per-push build: `BATON_BIN` override > `baton` on PATH (mise shim) > ephemeral `mise x github:alexey1312/baton@latest` — so the gate runs the **released** baton (CI-consistent), not an in-progress local build; set `BATON_BIN=.build/release/baton` to dogfood unreleased changes. NB: a stale absolute `core.hooksPath` (e.g. after a repo-dir rename) silently disables all hooks — re-run `mise run setup`.
